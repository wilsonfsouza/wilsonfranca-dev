...
export function SocialMediaBar({ children, ...rest }: SocialMediaBarProps) {
  return (
    ...
    >
      {children}
      <HStack gap="1.5rem" alignItems="center" justifyContent="space-between">
        <ExternalLink href="https://github.com/wilsonfsouza">
          <Icon as={FaGithub} fontSize="2.25rem" aria-label="Github" />
        </ExternalLink>

        <ExternalLink href="https://www.linkedin.com/in/wilsonfsouza/?locale=en_US">
          <Icon as={FaLinkedin} fontSize="2.25rem" aria-label="Linkedin" />
        </ExternalLink>
      </HStack>
    </Stack>
  );
